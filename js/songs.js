...
function getTypeBadgeHtml(song) {
  const typeRaw = (song.type || 'single').toLowerCase();
  const label   = typeRaw === 'album' ? 'Album' : typeRaw === 'ep' ? 'EP' : 'Single';
  const cls     = `badge badge--type badge--type-${typeRaw}`;
  const tracks  = song.track_count ? `<span class="badge-tracks">${song.track_count} lagu</span>` : '';
  return `<span class="${cls}">${label}${tracks}</span>`;
}
  ...
  tbody.innerHTML = songs.map(song => `
    <tr>
      ...
      <td>
        ...
        <div class="td-secondary">${escapeHtml(song.slug)}${song.isrc ? ` · ${escapeHtml(song.isrc)}` : ''}</div>
      </td>
      <td>${getTypeBadgeHtml(song)}</td>
      <td>${formatDate(song.release_date)}</td>
      <td>
        ${song.status === 'upcoming' || song.coming_soon
          ? `<span class="badge badge--coming-soon">Coming Soon</span>`
          : `<span class="badge badge--released">Released</span>`}
        ${song.featured ? `<span class="badge badge--featured">★ Latest</span>` : ''}
      </td>
      <td class="td-actions">
...
function updateTrackCountVisibility(type) {
  const group = document.getElementById('track-count-group');
  if (!group) return;
  const show = type === 'ep' || type === 'album';
...
function resetForm() {
  ...
  // Reset type to single and hide track count
  const typeEl = document.getElementById('song-type');
  if (typeEl) { typeEl.value = 'single'; updateTrackCountVisibility('single'); }
}
...
function populateForm(song) {
  const fields = [
    'title', 'slug', 'release_date', 'spotify_url', 'youtube_url',
    'apple_music_url', 'presave_url', 'description', 'isrc', 'duration', 'genre_tags',
  ];
  fields.forEach(field => {
    const elId = 'song-' + field.replace(/_/g, '-');
  ...
  // Type select
  const typeEl = document.getElementById('song-type');
  if (typeEl) typeEl.value = (song.type || 'single').toLowerCase();

  // Track count
  const trackCountEl = document.getElementById('song-track-count');
  if (trackCountEl) trackCountEl.value = song.track_count ?? '';

  // Checkboxes
  const comingSoon = document.getElementById('song-coming-soon');
  if (comingSoon) comingSoon.checked = song.coming_soon ?? (song.status === 'upcoming') ?? false;

  const featured = document.getElementById('song-featured');
  if (featured) featured.checked = song.featured ?? false;
  ...
  // Show/hide track count field based on type
  updateTrackCountVisibility(typeEl?.value);
}
...
const isFeatured   = document.getElementById('song-featured')?.checked ?? false;
const typeVal      = document.getElementById('song-type')?.value || 'single';
const trackCount   = document.getElementById('song-track-count')?.value;
const genreTags    = document.getElementById('song-genre-tags')?.value?.trim() || null;
const appleMusicUrl = document.getElementById('song-apple-music-url')?.value?.trim() || null;
const presaveUrl   = document.getElementById('song-presave-url')?.value?.trim() || null;
...
const payload = {
  ...
  // Status fields — keep both old + new in sync
...
export function initSongs() {
  ...
  // Type select — show/hide track count field
  const typeSelect = document.getElementById('song-type');
  if (typeSelect) {
    typeSelect.addEventListener('change', () => updateTrackCountVisibility(typeSelect.value));
    updateTrackCountVisibility(typeSelect.value);
  }
